...
```{r}
# 1. 读取 CSV 文件
df_chem <- read.csv("D:/msc_projecct/QC_saliva/QC_saliva/KGCO-0504-20DSMLTA+ Saliva Data Tables_Chemical Annotation.csv", stringsAsFactors = FALSE)
df_metabolites <- read.csv("D:/msc_projecct/QC_saliva/QC_saliva/metabolon_saliva_batch_norm_common_metab_quantile_normalised_at_least_80perc_imputed_2021.csv", stringsAsFactors = FALSE)
```
...
```{r}
# 9. 保存筛选后的数据
write.csv(df_filtered, "D:/msc_projecct/QC_saliva/QC_saliva/filtered_named_metabolites.csv", row.names = FALSE)
```
